...
public class Basics : MonoBehaviour
{
	...
	IEnumerator Start()
	{
		...
		EaseFunction innerEase = EaseFactory.StandardEaseFunction(Ease.Linear);
		EaseFunction easeFunc = EaseFactory.StopMotion(3, innerEase);
		redCube.DOMove (new Vector3 (0, 4, 0), 2).SetEase(easeFunc);

		// Let's move the green cube FROM 0,4,0 in 2 seconds
		greenCube.DOMove(new Vector3(0,4,0), 2).From().SetEase(EaseFactory.StopMotion(5));
		...
		purpleCube.GetComponent<Renderer>().material.DOColor(Color.yellow, 2).SetLoops(-1, LoopType.Yoyo).SetEase(EaseFactory.StopMotion(4));
	}
}